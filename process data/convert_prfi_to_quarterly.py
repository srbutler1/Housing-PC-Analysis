...
# Read the CSV file
df = pd.read_csv('PRFI.csv')

# Convert observation_date to datetime
df['observation_date'] = pd.to_datetime(df['observation_date'])

# Create a new dataframe with quarterly dates and calculate mean for each quarter
quarterly_df = df.set_index('observation_date').resample('Q')['PRFI'].mean().reset_index()

# Calculate percent change between quarters
quarterly_df['pct_change'] = quarterly_df['PRFI'].pct_change() * 100

# Round values to 2 decimal places
quarterly_df['PRFI'] = quarterly_df['PRFI'].round(2)
...
# Save to new CSV
quarterly_df.to_csv('PRFI_quarterly.csv', index=False)

# Print validation information
print("Conversion complete. Data saved to PRFI_quarterly.csv")
